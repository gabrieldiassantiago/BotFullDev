...
export const diversao = async (c, mensagemBaileys, botInfo) => {
    ...
    try {
        switch (comandoSemPrefixo) {
            case "glubglub":
                try {
                    // Defina o número específico que será marcado
                    const numeroGlubGlub = "5512996839077@s.whatsapp.net"; // Substitua pelo número desejado

                    if (numeroGlubGlub) {
                        await socket.responderComMencoes(
                            c,
                            id_chat,
                            criarTexto(
                                comandos_info.grupo.glubglub.msgs.resposta,
                                numeroGlubGlub.replace("@s.whatsapp.net", "")
                            ),
                            [numeroGlubGlub],
                            mensagem
                        );
                        ...
                        await socket.responderTexto(
                            c,
                            id_chat,
                            comandos_info.grupo.glubglub.msgs.sem_numero,
                            mensagem
                        );
                    }
                } catch (err) {
            ...
            case 'mascote':
                try {
                    const mascoteFotoURL = "https://media-gru2-2.cdn.whatsapp.net/v/t61.24694-24/466429386_940620341316584_2009887981966824343_n.jpg?ccb=11-4&oh=01_Q5AaIC05JbgqlF2n_AC0M88XF-oF5S6B74lTzly8ny6nn4h9&oe=67911D57&_nc_sid=5e03e0&_nc_cat=110"
